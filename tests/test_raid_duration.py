...
@pytest.mark.asyncio
async def test_hour_level_choice_allows_low_level_when_cap_is_reserved(tmp_path):
    db.reset_for_tests(str(tmp_path / "t.db"))
    raid_id = db.create_raid(
        name="Gigalodon",
        date_iso="2026-06-28",
    ...
    cog = _cog()
    cog._refresh_hour_poll_message = _async_noop

    await cog.handle_level_choice(interaction, raid_id, LEVEL_199_MINUS, "hour", 15)

    assert interaction.response.edits[0][0].startswith("Palier enregistré : **199-**.")
    assert db.get_level_choice(raid_id, 30) == LEVEL_199_MINUS
    assert db.get_user_votes(raid_id, 30, "hour") == ["15"]
